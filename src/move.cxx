...
Position_set::Position_set(std::initializer_list<value_type> elements)
{
...
size_t Position_set::size() const
{
    return bits_.count();
}
...
Position_set& Position_set::operator&=(Position_set that)
{
    bits_ &= that.bits_;
    return *this;
}
...
Position_set Position_set::operator&(Position_set that) const
{
...
Position_set Position_set::operator~() const
{
    Position_set result(*this);
...
bool operator==(Position_set a, Position_set b)
{
...
size_t Position_set::index_of_checked_(value_type p)
{
...
size_t Position_set::index_of_(value_type p)
{
...
bool Position_set::iterator::stopping_point_(size_t index) const
{
    return index == index_limit || bits_->test(index);
}

bool operator==(Position_set::iterator a, Position_set::iterator b)
{
    return *a == *b;
}
...
std::ostream& operator<<(std::ostream& os, Position_set pset)
{
    os << "{";

    bool first = true;
    for (ge211::Position pos : pset) {
